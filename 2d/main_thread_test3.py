...
def cam_read_undi(side,d):
    ...
    while True:   
        ...
        if side == 'front':                   
            ...
            d['left_undi'] = cv2.remap(d['left_frame'], map1, map2, interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT) 
        ...
        prev_time = curr_time
        fps_string = f'카메라 + 왜곡보정 == {side}  ===> term = {term:.3f},  FPS = {fps:.2f}'
        print(fps_string)
...
def make_top(side, d):
    ...
    while True:
        ...
        if False not in [d['front_frame_check'], d['back_frame_check'], d['left_frame_check'], d['right_frame_check']]:
            ...
            prev_time = curr_time
            fps_string = f'make top == {side}  ===> term = {term:.3f},  FPS = {fps:.2f}'
            print(fps_string)
...
if __name__ == '__main__':
    ...
    front_read_undi = multiprocessing.Process(target=cam_read_undi,args=('front',d))
    back_read_undi = multiprocessing.Process(target=cam_read_undi,args=('back',d))
    left_read_undi = multiprocessing.Process(target=cam_read_undi,args=('left',d))
    right_read_undi = multiprocessing.Process(target=cam_read_undi,args=('right',d))
    front_top = multiprocessing.Process(target=make_top,args=('front',d))
    back_top = multiprocessing.Process(target=make_top,args=('back',d))
    left_top = multiprocessing.Process(target=make_top,args=('left',d))
    right_top = multiprocessing.Process(target=make_top,args=('right',d))
    make_bev = multiprocessing.Process(target=birdeye,args=('bev',d))
    show = multiprocessing.Process(target=show_bev,args=('bev',d))

    make_bev.start()
    
    front_read_undi.start()
    back_read_undi.start()
    left_read_undi.start()
    right_read_undi.start()
    ...
    processes = [front_read_undi, back_read_undi, left_read_undi, right_read_undi, front_top, back_top, left_top, right_top, make_bev,show]
