...
while(True):
    ...
    ret1, front = cap1.read()    # Read 결과와 frame
    ret4, left = cap2.read()
    ret3, right = cap3.read()
    ret2, back = cap4.read()

    if(ret1) :
        # cam1 = undi_top(frame1, 'right')         
        cv2.imshow('front', front) 
        undistorted_front = undistort(front, 1.5)
    ...
    if(ret2) :
        ...
        undistorted_back = undistort(back, 1.5)
    ...
    if(ret3) :
        ...
        undistorted_right = undistort(right, 1.5)
        # cv2.imshow('frame_3', undistorted_img3) 
        pass

    if(ret4) :
        cv2.imshow('left', left) 
        undistorted_left = undistort_left(left, 1.5)
    ...
    if key == ord('z'):
        cv2.imwrite('front' + str(num) + '.png', front)
        cv2.imwrite('back' + str(num) + '.png', back)
